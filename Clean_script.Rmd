...
```{r}
colour_per_outreak=base_world+ggtitle('')+geom_jitter(data=counlist,position=position_jitter(width=1.4, height=1.4),aes(x=lon, y=lat, color=Year,size=n),alpha=0.6,stroke=0.9)+labs(colour='Outbreak years')+theme(legend.title = element_text(size = 15),legend.text =element_text(size = 14) )+scale_size_continuous(range = c(5,17),breaks = c(round(min(counlist$n)),round(mean(counlist$n)),round(max(counlist$n))))
colour_per_outreak
```
...
###Saving:
```{r}
ggsave(filename ='map.pdf',plot=colour_per_outreak,height = 21,width = 21)
```
